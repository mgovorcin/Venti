...
def main(iargs=None):
    ...
    f = open(str(out.parent / 'drop_ix.txt'), 'w')
    f.write(f'# Drop ix: {drop_df.index.values}\n')
    f.write('\nIndex    Date12        Date2     Date1    Aria_date12\n')
    f.close()
    drop_df['index'] = drop_df.index.values
    aria_d12 = [str(ix.date2) + '_' + str(ix.date1) for _, ix in drop_df.iterrows()]
    drop_df['aria_date12'] = aria_d12
    drop_df.to_csv(str(out.parent / 'drop_ix.txt'), columns=['index', 'date12', 'date2', 'date1', 'aria_date12'],
