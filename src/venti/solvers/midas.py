...
# Supress Runtime Warning due to all NaN in array
...
def midas(dates, disp, steps=None, tolerance=0.001, print_msg=False, display=False):
    ...
    NOTES : Authors state that MIDAS does not necessarily represent strain accumulation
    ...
    f_ix, b_ix = select_pairs(
        dates, steps=steps, tolerance=tolerance, print_msg=print_msg)
    ...
    sigma = 1.4826 * \
        np.nanmedian(np.absolute(vp - v_dot), axis=0, keepdims=True)
    ...
    if display is True:
        plot_midas(dates, disp*1000, vel*1000,
                   vel_std*1000, intercept*1000, steps)
...
def select_pairs(dates, steps=None, tolerance=0.001, print_msg=False):
    ...
    for _, direction in enumerate(['forward', 'backward']):
        ...
        pairs_ix = np.where((date_diff > (1.0 - tolerance))
                            & (date_diff < (1.0 + tolerance)))
        ...
        skipped_ix = (skipped_pairs[skipped_ix[0]
                      [index]], skipped_ix[1][index])
...
def date_difference_ydec(date1, date2):
    ...
    if date1.dtype != date2.dtype:
        raise ValueError(
            f'Different dtype between date1: {date1.dtype} and date2: {date2.dtype}')
...
def plot_midas(dates, disp, vel, vel_std, intercept, steps=None):
    '''
    ...
    plot = [(ax.plot(dates, disp[:, i] - np.mean(disp[:, i]), 'b.'),
             ax.plot(dates, (intercept[0, i] + np.squeeze(vel)[i] * date_dt) - np.mean(intercept[0, i] + np.squeeze(vel)[i] * date_dt),
            'r-', label=f'MIDAS v {np.squeeze(vel)[i]:.2f} \u00B1 {np.squeeze(vel_std)[i]:.2f} mm/yr'))
            for i, ax in enumerate(axs)]

    if steps.any():
        [ax.vlines(steps, np.min(disp[:, i] - np.mean(disp[:, i])), np.max(disp[:, i] - np.mean(disp[:, i])), linestyles='dashed')
         for i, ax in enumerate(axs)]
