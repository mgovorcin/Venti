...
class local_cluster:
    ...
    def run(self, func, func_data, results, verbose=True):
        ...
        futures, submission_time = self.submit_job(func, func_data, sub_boxes, verbose=verbose)

        # assemble results from all workers
        results = self.collect_result(futures, results, submission_time, verbose=verbose)

        return results
    
    def submit_job(self, func, func_data, sub_boxes, verbose=True):
        vprint = print if verbose else lambda *args, **kwargs: None 
        submission_time = time.time()
        futures = []

        for i, sub_box in enumerate(sub_boxes):
            vprint(f'submit a job to the worker for sub box {i}: {sub_box}')
            func_data['box'] = sub_box

            func_data_new = slice_func_data(func_data)
            vprint('Spliting the func data')
            vprint(func_data['data'].shape, '>', func_data_new['data'].shape)
    ...
    def collect_result(self, futures, results, submission_time, verbose=True):
        ...
        for future, sub_results in as_completed(futures, with_results=True):
            ...
            vprint("FUTURE #{} complete. Time used: {:.0f} seconds".format(num_future, sub_t))
            ...
            for i, sub_result in enumerate(sub_results[:-1]):
                ...
                if sub_result is not None:
                    ...
                    if num_dim == 2:
                        box = sub_box[1:] #reject the first dimension ndim

                    if num_dim == results[i].ndim:
                        results[i][box] = sub_result
